...
export async function handler(event) {
    ...
    try {
        ...
        const weelockAuthentication = null;
        // // Checking authorizations
        // request.uri = checkAuthorization(weelockAuthentication, request);
        // request.uri = resolveUri(request.uri);
        // // Resolving build version
        // request.uri = resolveBuildVersion(weelockAuthentication, request);
        const tokens = request.uri.split("/")
        const token = tokens[tokens.length - 1]
        if (token.includes(".")) {
            request.uri = "/" + frontBuildDefault + request.uri;
        } else {
            request.uri = "/" + frontBuildDefault + request.uri + "/index.html";
        }
        // request.uri = "/" + frontBuildDefault + request.uri;
        // Send to origin
        return request;
